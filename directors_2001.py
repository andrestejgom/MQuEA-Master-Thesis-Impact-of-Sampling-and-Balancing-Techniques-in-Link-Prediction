...
import sys
sys.stdout = open('directors_2001_link_prediction.txt', 'w')
...
# Configuration
NETWORK_PATH = "directors_meetings/directors_2001"
RESULTS_DIR = "directors_2001_predictions"
RANDOM_STATE = 42
...
def shortest_path_similarity(G, ebunch=None):
    """
    Shortest-Path Similarity (from CCPA with alpha=0).

    Computes the score based solely on shortest-path proximity:
        score(u,v) = N / d(u,v)
    ...
    # Compute shortest-path similarity using CCPA with alpha=0
    sp_scores = {
        (u, v): score 
        for u, v, score in nx.common_neighbor_centrality(G, ebunch=edge_pairs, alpha=0)
    }
    
    # Yield scores for each pair
    for u, v in edge_pairs:
        yield (u, v, sp_scores.get((u, v), 0))
...
def dp_shortest_path_similarity(G, ebunch=None):
    """
    Enhanced Shortest-Path Similarity with Degree of Popularity (DP).

    Original Shortest-Path Similarity (from CCPA with alpha=0):
        score(u,v) = N / d(u,v)
        where d(u,v) is the shortest path length between nodes u and v.

    Enhanced version:
        S-P similarity + Degree of Popularity (DP)
    """
    if ebunch is None:
        ebunch = nx.non_edges(G)
    ...
    # Original shortest-path similarity
    sp_scores = {
        (u, v): score 
        for u, v, score in nx.common_neighbor_centrality(G, ebunch=edge_pairs, alpha=0)
    }
    
    # Degree of Popularity (DP) scores
    dp_scores = {
        (u, v): score 
        for u, v, score in degree_of_popularity(G, ebunch=edge_pairs)
    }
    
    # Combine both measures
    for u, v in edge_pairs:
        sp_score = sp_scores.get((u, v), 0)
        dp_score = dp_scores.get((u, v), 0)
        
        enhanced_score = sp_score + dp_score
...
LINK_PREDICTION_METHODS = {
    ...
    'shortest_path_similarity': shortest_path_similarity,
    'dp_shortest_path_similarity': dp_shortest_path_similarity,
...
def save_experiment_results(all_results, network_info):
    ...
    # Save to Excel file
    excel_file = os.path.join(RESULTS_DIR, "directors_2001_results.xlsx")
...
def create_visualizations(summary_df, network_info):
    ...
    color_palette = {
        ...
        'shortest_path_similarity': '#27AE60',
        'dp_shortest_path_similarity': '#1E8449'
    }
    
    marker_styles = {
        'preferential_attachment': 'o',
        'dp_preferential_attachment_scaled': '^',
        'shortest_path_similarity': 's',
        'dp_shortest_path_similarity': 'D'
    }
    ...
    method_labels = {
        ...
        'shortest_path_similarity': 'Shortest Path Similarity',
        'dp_shortest_path_similarity': 'DP-SP Similarity'
    }
    ...
    for sampling_method in sampling_methods:
        ...
        legend = ax1.legend(loc='upper right', fontsize=12, framealpha=0.95, 
                           fancybox=True, shadow=True, ncol=1, columnspacing=1.5)
        legend.get_frame().set_facecolor('#f8f9fa')
        legend.get_frame().set_edgecolor('#dee2e6')
        ...
        fig1.tight_layout()
        plot_file1 = os.path.join(RESULTS_DIR, f"directors_2001_aupr_if_predictions_{sampling_suffix}.png")
        ...
        fig2.tight_layout()
        plot_file2 = os.path.join(RESULTS_DIR, f"directors_2001_auc_predictions_{sampling_suffix}.png")
        ...
        fig3.tight_layout()
        plot_file3 = os.path.join(RESULTS_DIR, f"directors_2001_aupr_predictions_{sampling_suffix}.png")
        ...
        method_order = [
            ...
            'shortest_path_similarity', 'dp_shortest_path_similarity'
        ]
        ...
        fig4.tight_layout()
        plot_file4 = os.path.join(RESULTS_DIR, f"directors_2001_aupr_if_table_{sampling_suffix}.png")
        ...
        fig5.tight_layout()
        plot_file5 = os.path.join(RESULTS_DIR, f"directors_2001_auc_table_{sampling_suffix}.png")
        ...
        fig6.tight_layout()
        plot_file6 = os.path.join(RESULTS_DIR, f"directors_2001_aupr_table_{sampling_suffix}.png")
...
def main():
    ...
    print("LINK PREDICTION EXPERIMENT: DIRECTORS MEETINGS 2001")
    print("="*60)
    ...
    network_info = {
        'path': NETWORK_PATH,
        'description': f"2001 network with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges",
